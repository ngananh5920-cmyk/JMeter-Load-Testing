...
	Test Plan
Test Plan là thành phần gốc của JMeter, dùng để quản lý toàn bộ kịch bản kiểm thử. Trong bài thực hành này, Test Plan bao gồm: Thread Group (Load Test API), HTTP Request (Get Post), View Results Tree và Summary Report.
...
	Thread Group
Thread Group dùng để mô phỏng người dùng ảo truy cập hệ thống. Dựa trên tổng số mẫu sinh ra trong Summary Report, kịch bản được cấu hình để thực hiện tổng cộng 150 requests.
...
	HTTP Request: HTTP Request được sử dụng để gửi yêu cầu đến API.
+	Method: GET
+	Protocol: https
+	Server Name: jsonplaceholder.typicode.com
+	Path: /posts 
	Response Assertion: Response Assertion dùng để kiểm tra dữ liệu trả về từ API có đúng kỳ vọng hay không.
+	Name: Check ID
...
(Dòng số 2 - No Label)	30	0	0	0	0.00	100.00%	3.3/sec	2.79	0.00	859.0
...
	Thu thập được số liệu trực quan để phân tích hiệu năng hệ thống. Mặc dù tỷ lệ lỗi (Error Rate) thực tế cao do giới hạn từ phía server demo, bài thực hành đã giúp em hiểu rõ tầm quan trọng của việc kiểm thử tải để phát hiện ra ngưỡng chịu đựng và điểm nghẽn của hệ thống API.

<img width="468" height="654" alt="image" src="https://github.com/user-attachments/assets/0ae2f7a1-d77a-4ae6-a80f-8ac6fa819915" />
